...
/// This is a hidden module that exists to emit a deprecation warning when the
/// "backtrace" feature is enabled.  It's used by the `assert_snapshot!` macro.
///
/// The warning generated by this hack looks like the following:
///
/// ```ignore
/// warning: use of deprecated module `insta::deprecated_backtrace_support`: You
/// are using insta with the deprecated "backtrace" feature. Remove this feature
/// from your Cargo.toml as it will go away in future insta versions. The
/// functionality provided by this feature is now available out of the box.
/// ```
#[doc(hidden)]
#[cfg_attr(
    all(
        feature = "backtrace",
        not(feature = "__private_hide_backtrace_deprecation_warning")
    ),
    deprecated(
