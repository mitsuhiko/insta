...
impl Content {
    ...
    fn from_json_blob(blob: JsonValue) -> Self {
        match blob {
            JsonValue::Null => Self::None,
            JsonValue::Boolean(b) => Self::from(b),
            JsonValue::Number(n) => match i64::try_from(n) {
            ...
            JsonValue::Short(s) => Self::from(s.to_string()),
            JsonValue::String(s) => Self::from(s),
            JsonValue::Array(seq) => {
                let seq = seq.into_iter().map(Self::from_json_blob).collect();
                Self::Seq(seq)
            }
            // This is probably a struct, but we don't have context on the struct name
            JsonValue::Object(obj) => {
                // TODO: why doesn't this have `.into_iter()`
                let obj = obj
                    .iter()
                    .map(|(k, v)| (Self::from(k), Self::from_json_blob(v.to_owned())))
                    .collect();
                Self::Map(obj)
            }
        }
    ...
    pub(crate) fn as_json(&self) -> BoxedResult<String> {
        let json_blob = self.as_json_blob()?;

        let mut buf = Vec::new();
        json_blob.write(&mut buf).unwrap();

        Ok(String::from_utf8(buf).expect("JSON is valid UTF-8"))
    }

    fn as_json_blob(&self) -> Result<JsonValue> {
        fn translate_seq(seq: &[Content]) -> Result<JsonValue> {
            let seq = seq
                .iter()
                .map(Content::as_json_blob)
                .collect::<Result<_>>()?;
            Ok(JsonValue::Array(seq))
        }

        fn translate_fields(fields: &[(&str, Content)]) -> Result<JsonValue> {
            let obj = fields
                .iter()
                .map(|(k, v)| Ok((k.to_owned(), v.as_json_blob()?)))
                .collect::<Result<_>>()?;
            Ok(JsonValue::Object(obj))
        }

        match self {
            Self::Bool(b) => Ok(JsonValue::from(*b)),
            Self::U8(n) => Ok(JsonValue::from(*n)),
            Self::U16(n) => Ok(JsonValue::from(*n)),
            Self::U32(n) => Ok(JsonValue::from(*n)),
            Self::U64(n) => Ok(JsonValue::from(*n)),
            // TODO: switch to a parser that supports u128/i128
            Self::U128(_) => Err(Error::UnsupportedDataType),
            Self::I8(n) => Ok(JsonValue::from(*n)),
            Self::I16(n) => Ok(JsonValue::from(*n)),
            Self::I32(n) => Ok(JsonValue::from(*n)),
            Self::I64(n) => Ok(JsonValue::from(*n)),
            // TODO: switch to a parser that supports u128/i128
            Self::I128(_) => Err(Error::UnsupportedDataType),
            Self::F32(f) => Ok(JsonValue::from(*f)),
            Self::F64(f) => Ok(JsonValue::from(*f)),
            Self::Char(c) => Ok(JsonValue::from(c.to_string())),
            Self::String(s) => Ok(JsonValue::from(s.as_str())),
            Self::Bytes(bytes) => {
                let bytes = bytes.iter().map(|b| JsonValue::from(*b)).collect();
                Ok(JsonValue::Array(bytes))
            }
            Self::None | Self::Unit | Self::UnitStruct(_) => Ok(JsonValue::Null),
            Self::Some(content) => content.as_json_blob(),
            Self::UnitVariant(_, _, variant) => Ok(JsonValue::from(*variant)),
            Self::NewtypeStruct(_, content) => content.as_json_blob(),
            Self::NewtypeVariant(_, _, variant, content) => {
                let mut obj = JsonObj::new();
                obj.insert(variant, content.as_json_blob()?);
                Ok(JsonValue::Object(obj))
            }
            Self::Seq(seq) | Self::Tuple(seq) | Self::TupleStruct(_, seq) => translate_seq(seq),
            Self::TupleVariant(_, _, variant, seq) => {
                let mut obj = JsonObj::new();
                obj.insert(variant, translate_seq(seq)?);
                Ok(JsonValue::Object(obj))
            }
            Self::Map(map) => {
                let map = map
                    .iter()
                    .map(|(k, v)| {
                        if let Self::String(k) = k {
                            Ok((k, v.as_json_blob()?))
                        } else {
                            Err(Error::InvalidStructField(k.to_owned()))
                        }
                    })
                    .collect::<Result<_>>()?;
            ...
            Self::Struct(_, fields) => translate_fields(fields),
            Self::StructVariant(_, _, variant, fields) => {
                let mut obj = JsonObj::new();
                obj.insert(variant, translate_fields(fields)?);
                Ok(JsonValue::Object(obj))
            }
        }
    }
